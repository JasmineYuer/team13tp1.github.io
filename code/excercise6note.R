...
plot(1:10, all.deltas, xlab = "Degree", ylab = "Test MSE", type = "l")
d.min <- which.min(all.deltas)
...
# annova: proves that model with degree of 3,2,9 are significant (small p value)
# CV: shows that the minimum cv error is at degree of 9. That's the difference 
# reason why: small p value in annova reject the null hypothesis that the complex model does not have too much improvemnet 
            # Thus, higher degree of the model, even though having lower cv error, will possibly suffering the overfitting in the test set 
...
# This line looks like explain the data and does not have the problem of overfitting 
